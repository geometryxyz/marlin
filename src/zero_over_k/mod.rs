...
use crate::zero_over_k::error::{to_pc_error, Error};
...
impl<F: PrimeField, PC: HomomorphicPolynomialCommitment<F>, FS: FiatShamirRng> ZeroOverK<F, PC, FS> {
    ...
    pub fn prove<R: Rng, VO: VirtualOracle<F>>(
        ...
        let prover_initial_state =
            PIOPforZeroOverK::<F, VO, FS>::prover_init(domain, concrete_oracles, virtual_oracle, &alphas)?;
        let verifier_initial_state =
            PIOPforZeroOverK::<F, VO, FS>::verifier_init(virtual_oracle, domain)?;
        ...
        let (_, prover_first_oracles, prover_state) =
            PIOPforZeroOverK::<F, VO, FS>::prover_first_round(prover_initial_state, rng)?;
        ...
        let (verifier_first_msg, verifier_state) =
            PIOPforZeroOverK::<F, VO, FS>::verifier_first_round(verifier_initial_state, &mut fs_rng)?;
        //------------------------------------------------------------------
        ...
        let (_prover_second_msg, prover_second_oracles, prover_state) =
            PIOPforZeroOverK::<F, VO, FS>::prover_second_round(&verifier_first_msg, prover_state, rng);

        let query_set = PIOPforZeroOverK::<F, VO, FS>::verifier_query_set(&verifier_state, alphas)?;
        //------------------------------------------------------------------
        ...
        let q2_commit = PC::multi_scalar_mul(
            &r_commitments,
            {
                let scalar = verifier_first_msg.c;
